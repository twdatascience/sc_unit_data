...
def import_excel_files(path: str = None) -> Dict[str, pd.DataFrame]:
    ...
    The path must be provided as an argument.
    """
    if path is None:
        raise ValueError("Please provide a path to an Excel file or a directory containing Excel files.")
...
def write_unit_aggregation_report(filtered_dict: Dict[str, pd.DataFrame]):
    ...
    The file is named as today's date + ' unit aggregation report.xlsx'.
    """
    today_str = datetime.date.today().strftime("%Y-%m-%d")
    filename = f"{today_str} unit aggregation report.xlsx"
